...
- 哨兵也是一个Redis服务器，但是只提供数据操作
- 配置哨兵的数量通常为单数（利于投票），3, 5, 7 ...

配置哨兵`sentinel.conf`

命令 | 说明
:- | :-
`redis-sentinel sentinel-端口号.conf` | 根据哨兵配置文件启动哨兵
...
> 集群用于每秒操作次数（OPS）过高、内存占用情况过高问题的处理
...
配置 | 说明
:- | :-
`cluster-enable yes` | 将当前的服务设置为集群中的一个节点
`cluster-config-file nodes-端口号.conf` | 设置配置文件
`cluster-node-timeout 50000` | 设置节点的超时时间，毫秒为单位

> 设置好配置文件后，通过src/目录下的`redis-trib.rb`命令来设置集群（需要ruby的支持，同时保证大版本号对应）
